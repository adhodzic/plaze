<template>
  <div class="container">
    ...
    <div class="display-4" style="margin-bottom: 50px;">{{post.title}}</div>
    ...
    <div class="comment-section">
      <div class="comment-box" v-if="comment.parentId == null" :key="comment.id" v-for="comment, i in this.comments" style="margin-bottom:20px; padding-bottom:20px;">
        ...
        <p style="margin-left: 30px;">{{comment.text}}</p>
        <div id="reply-all" @click="$set(closedItems, i, !closedItems[i])" class="replay">
          <i id="reply-all-img" class="fas fa-reply-all"></i>
        </div>
        <div style="height: 15px; width: 15px; margin-left: 30px;" v-if="id_localStorage==comment.commentedBy._id" @click="deleteComment(comment._id)">
          <i id="delete-com" class="fas fa-trash-alt"></i>
        </div>
        <div v-if="closedItems[i]" style="padding-left:5em">
          <div style="margin-bottom:20px; background-color:rgb(122, 177, 255); border-radius:8px; word-wrap: break-word; padding: 10px;" :key="replay.id" v-for="replay in comment.replays">
            <p><b>{{replay.commentedBy["name"]}}</b> {{replay.text}}</p>
            <div @click="deleteComment(replay._id)">
              <i v-if="id_localStorage == replay.commentedBy._id" id="delete-reply"  class="fas fa-trash-alt"></i>
              <!-- <hr> -->
            </div>
          </div>
          <div>
            <form class="comment-input" @submit.prevent="NewReplay(comment._id)">
                ...
                <button :disabled="comment_btn" id="comment-button-reply" type="submit" class="btn btn-primary">Reply</button>
          </form>
          </div>
        </div>
      </div>
     ...
     <form id="comment-form" @submit.prevent="newComment()">
          ...
          <button :disabled="comment_btn" id="comment-button" type="submit" class="btn btn-primary">Comment</button>
    </form>
    </div>
  </div>
</template>
